...
% Plots
    ...
    % fault
        ...
        plot(step, f(1,:), Color=colors(1,:), LineStyle='-', LineWidth=2, DisplayName='f_1(k)');
        plot(step, f(2,:), Color=colors(2,:), LineStyle='-', LineWidth=2, DisplayName='f_2(k)');
        %plot(step, f(3,:), Color=colors(3,:), LineStyle='-', LineWidth=1, DisplayName='f_3(k)');
        legend();
        ylabel('Amplitude');
        grid on;
    ...
    % residuals
        ...
        plot(step(s+1:N), sum(r3'*r3, 1), Color=colors(3,:), LineStyle=':', LineWidth=2, DisplayName='r_3(k)');
        legend();
        xlabel('Time sample [k]');
        ylabel('Amplitude');
        grid on;
...
%% Combined error
    % Signals
        f = zeros(3, N);
        f(1, floor(N/5):floor(3*N/5)) = 2; % step fault on first sensor
        f(2, floor(2*N/5):floor(4*N/5)) = 1.2; % step fault on second sensor
...
% Plots
    figure('Name', 'Isolation Combined');
    colors = get(gca, 'ColorOrder');
    % fault
        subplot(211);
        title('Faults');
