...
\begin{equation}
    \begin{cases}
        ...
        \ln \hat{\lambda} (y) [k] = y^T[k] P y[k]\\
        \ln \hat{\lambda}_{\text{windowed}} (y) = \sum_{i=0}^{N_w} \ln \hat{\lambda} (y) [k-i]
    \end{cases}
\end{equation}
...
\begin{table}[htbp]
    \centering
    \caption{Percentage of samples above threshold depending on the window length for the quadratic criterion (\ref{eq:decision_criterion}) and the GLR criterion.}
