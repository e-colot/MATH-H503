...
\begin{equation}
    \hat{x}[k+1] = A\hat{x}[k] + B u[k] + L \left(y[k] - C\hat{x}[k] - D u[k]\right).
\end{equation}
...
\begin{equation}
    \begin{aligned}
        ...
        &\Leftrightarrow r[k] = y[k] - C \hat{x}[k] - D u[k]
    \end{aligned}
\end{equation}
...
The objective is to perform fault detection and isolation on the output signal of a tree-phased signal, which model is derived in Appendix \ref{appendix:three_phased_model}. A first set of observers were built using the following incidence matrix:
